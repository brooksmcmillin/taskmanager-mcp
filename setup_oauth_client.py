...
async def register_oauth_client(
    ...
    if redirect_uris is None:
        redirect_uris = [
            "https://mcp-auth.brooksmcmillin.com/oauth/callback"
        ]
...
async def list_oauth_clients(taskmanager_url: str, session_cookie: str) -> list[dict]:
    """
...
def main(
    ...
    async def run():
        try:
            ...
            print(f"\n💡 Usage:")
            print(f"Start the MCP auth server with:")
            print(f"python auth_server.py --taskmanager-url {taskmanager_url} --client-id {client_info['client_id']} --client-secret {client_info['client_secret']}")
